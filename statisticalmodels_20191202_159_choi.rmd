...
in the urn model p is a fixed parameter and it does not make sense to talk about probability. With Bayesian statistics, we model p as random variable and thus a statement such as “90% chance of winning” is consistent with the approach.
...
한 사람을 무작위로 골랐을 때 양성반응이 나왔다. 그렇다면 병에 걸릴 확률은 얼마인가? > $Prob(D=1|+)$
단, Prob(D = 1) = 0.00025
**$Bayes theorem : Pr(A|B) = Pr(B|A)*Pr(A)/Pr(B)$**

sol)
Prob(D=1|+)=P(+|D=1)*P(D=1)/Pr(+) = 0.02

환자일때 양성일 가능성은 0.99지만 양성일때 환자일 가능성은 0.02밖에 안됨
why? 랜덤으로 고른 사람이 병에 걸렸다는 매우 적은 확률에 대해서 계산을 하였으므로 상반된 결과 발생
...
조세의 타율이 20번 휘두르니 0.450이다. 진짜일까?
1) frequentist techniques를 이용하여 이항분포로 계산을 하니 confidence interval이 너무 크고 말도 안됬다.
-> 다른 선수들의 결과와 비교하는 hierarchical models를 사용하여 해결해보자
...
```{r}
ave <- sum(polls$spread)/nrow(polls)
SE <- sqrt(ave*(1-ave)/nrow(polls))
results <- c(ave, SE)
```
...
12. 12. Now use sapply function to change the prior variance from `seq(0.05, 0.05, len = 100)` and observe how the probability changes by making a plot.
```{r}
my_function <- function(N){
  B <- SE^2/(SE^2 + N^2)
  EX <- (1-B)*ave
  sd <- sqrt(1/(1/SE^2+1/N^2))
  1-pnorm(0, EX, sd)
}

prob <- seq(0.05, 0.5, len = 100)
y <- sapply(prob, my_function)
plot(prob, y)
```
...
Pollsters tend to make probabilistic statements about the results of the election. For example, “The chance that Obama wins the electoral college is 91%” is a probabilistic statement about a parameter which in previous sections we have denoted with  
d. We showed that for the 2016 election, FiveThirtyEight gave Clinton an 81.4% chance of winning the popular vote. To do this, they used the Bayesian approach we described.
...
### 16.8.3 Mathmatical representation of models
Suppose we are collecting data from one pollster and we assume there is no general bias. The pollster collects several polls with a sample size of N, so we observe several measurements of the spread X1,…,XJ. The theory tells us that these random variables have expected value d and standard error $2 \sqrt{p(1-p)/N}$. Let’s start by using the following model to describe the observed variability:

$X_j = d + \varepsilon_j.$

We use the index j to represent the different polls and we define εj to be a random variable that explains the poll-to-poll variability introduced by sampling error. To do this, we assume its average is 0 and standard error is $2 \sqrt{p(1-p)/N}$. If d is 2.1 and the sample size for these polls is 2,000, we can simulate J=6 data points from this model like this:
```{r}
set.seed(3)
J <- 6
N <- 2000
...
```
Now suppose we have J=6 data points from  
I=5 different pollsters. To represent this we now need two indexes, one for pollster and one for the polls each pollster takes. We use Xij with i representing the pollster and j representing the j-th poll from that pollster. If we apply the same model, we write:
...
The model above does not account for pollster-to-pollster variability. To fix this, we add a new term for the pollster effect. We will use hi to represent the house effect of the i-th pollster. The model is now augmented to:
$X_{i,j} = d + h_i + \varepsilon_{i,j}$

To simulate data from a specific pollster, we now need to draw an hi and then add the εs. Here is how we would do it for one specific pollster. We assume σ h is 0.025:
```{r}
I <- 5
J <- 6
N <- 2000
...
Since the same  
b
  is in every measurement, the average does not reduce the variability introduced by the b term. This is an important point: it does not matter how many polls you take, this bias does not get reduced.
...
```{r}
cis <- mutate(polls, Xhat = (spread+1/2), SE = sqrt(Xhat*(1-Xhat)/samplesize), lower = Xhat -1.96*SE, upper = Xhat+1.96*SE)

cis <- cis %>% select(state, startdate, enddate, pollster, grade, spread, lower, upper)
```
...
```{r}
actual <- (cis$actual_spread+1)/2

confidence <- cis %>% mutate(actual = actual,
test = cis$actual >= cis$lower & cis$actual <= cis$upper)
...
```{r}
confidence %>% group_by(pollster) %>% filter(n() >= 5) %>% summarise(ranking = mean(is.na(test)), n=n(), grade = grade[1]) %>% arrange(desc(ranking))
```


4. Repeat exercise 3, but instead of pollster, stratify by state. Note that here we can’t show grades.
```{r}
states <- cis %>% mutate(actual = actual,
test = cis$actual >= cis$lower & cis$actual <= cis$upper) %>% group_by(state) %>% filter(n() >= 5) 


states %>% summarise(ranking = mean(!is.na(test)), n=n()) %>% arrange(desc(ranking))
```


5. Make a barplot based on the result of exercise 4. Use `coord_flip`.
```{r}
states %>% ggplot(aes(state, test)) + geom_bar(stat = "identity") + coord_flip()
```
...
However, if the data in the urn is known to follow a normal distribution, then we actually have mathematical theory that tells us how much bigger we need to make the intervals to account for the estimation of  
σ. Using this theory, we can construct confidence intervals for any N. But again, this works only if **the data in the urn is known to follow a normal distribution**. So for the 0, 1 data of our previous urn model, this theory definitely does not apply.
...
If we are willing to assume the pollster effect data is normally distributed, based on the sample data X1,…,XN,
```{r}
