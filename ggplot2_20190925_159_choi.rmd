...
```
benefit:
1. more intuitive for beginners is that it uses a grammar of graphics, the _gg_ in **ggplot2**.
2.  its default behavior is carefully chosen to satisfy the great majority of cases and is visually pleasing.

**ggplot2** is designed to work exclusively with data tables in **tidy format**.



# 8.1 The components of a graph
**data, geomery(그래프 종류), aesthetic mapping(표현되는 거의 모든것)**
...
**Layer: 일종의 정보, 추가 가능함**
**`+`: layer 추가**
  **ex) data %>% ggplot() + layer1 + layer2...**

**geometry 정하기: `geom_...`**
  **ex) geom_point: scatterplot**

**`help`의 **Aesthetic session**: 필수 지정 요소 무엇인지 알려줌**
...
**`aes`** 
**: data를 graph의 feature와 연결** 
**:`geom_...`의 argument로 주로사용(x,y열, 색깔...)**
 **ㄴaesthetix mapping: data가 어떻게 그래프에  표현되는지..?**
 **ㄴex)murders %>% ggplot() +**
 **geom_point(aes(x+population/10^6, y = total))**
 **_x,y 생략가능, data column name $없이 인식_**
...
```
The scale and labels are defined by default when adding this layer. Like **dplyr** functions, `aes` also uses the variable names from the object component: 
we can use `population` and `total` without having to call them as `murders$population` and `murders$total`. The behavior of recognizing the variables from the data component is quite specific to `aes`. 
With most functions, if you try to access the values of `population` or `total` outside of `aes` you receive an error.



# 8.5 Layers
**`geom_txt(geom_label)` : each point 에 label 붙임**
 **ㄴex)p + geom_text(aes(population, total, label = abb))**
**point 크게 : `size` argument(mapping X)**
 **ㄴmapping:aes()포함,** 
           **use data from specific overvation** 
**label 옆으로 : `nudge_x` argument**
 **ㄴex) p + geom_point(size=3)+**
       **geom_text(nudge_x=1)**
...
**`ggplot argument에 지정`**
**:모든 layer에 default로 저장**
**덮어쓰기도 가능**
**:뒤의 lable에서 따로 mapping**
...
**:스케일 바꿈, argument: `trans =`**
**ㄴ`scale_x_log10()("y")` : 이미 지정되있는 function**
...
**`col`**
**:argument of `geom_point`, 색을 바꿈**
**:categorial variable 지정 > 자동으로 범주별로 color 다르게 적용**
**`show.legend = FALSE` **
**:geom_point 의 argument**
**:옆의 색범례 없앰**
...
**aesthetic mapping 으로 directly 추가할 수 없는 것들에 대하여..**
**`geom_abline()`:1차함수 선넣기**
**:argument > intercept(상수), slope(기울기)**
 **default is 0,1 for each**
 **also, ity(style), color is arguments of this**
**`scale_color_discrete(name=sth)`**
**:sth으로 색범례 title 바꿈**
...
y=rx, with  y and x our axes: 
total murders and population in millions respectively. 
In the log-scale this line turns into:  
...
**ggplot2는 ggthemes, ggrepel같은 packages를 add할 수 있다**
**1.theme:**
**`ds_theme_set`: defalt theme 결정**
**`theme_...`:theme 바꿈**
**2.ggrepel:label 관련**
**`geom_text_label`: `geom_text`대신 사용가능**
...
#library(ggrepel)
murders %>% ggplot(aes(population/10^6, total, label = abb, col = region)) + geom_text_repel() + geom_point()
```
...
# 8.13 Quich plots with `qplot`
**`qplot`:ggplot approach 로 plot 만들고 싶을 때**

We have learned the powerful approach to generating visualization with ggplot. However, there are instances in which all we want is to make a quick plot of, for example, a histogram of the values in a vector, a scatter plot of the values in two vectors, or a boxplot using categorical and numeric vectors. 
...
# 8.14 Grids of plots
**when want to show two graphs together: `grid.arrange(graph1, graph2, ncol=2)`**
