...
#### 2-1. SIFT와 PolyPhen
우선 이 두개를 비교해보기로 했다. 두가지 column은 모두 단백질 아미노산의 치환과 그것이 표현형에 끼치는 영향력과 관련된 variant이다. 정의가 비슷한 이 두 variant관의 관계가 궁금했다.
또한 개인적으로 column들을 조사하다 찾은 아래의 주소에서 두 variant가 매우 유사하다는 정보를 확인할 수 도 있었다.(https://www.liebertpub.com/doi/full/10.1089/gtmb.2010.0036)
...
d1 %>% ggplot(aes(as.numeric(sift_n), as.numeric(poly_n), col = poly_c)) + geom_point(size = 0.5) + labs(x='SIFT', y='PolyPhen', title = 'Relationship between SIFT&PolyPhen', color = "PolyPhen")
```
...
MPC역시 SIFT, PolyPhen과 같은 측정방법의 일종이다. 따라서 3-1에서의 방법을 사용하여 세 variable을 비교하면 된다고 생각하여 아래처럼 그래프를 그려보았다..
```{r echo=TRUE}
...
1. MPC값에 따라 sift와 polyphen의 값의 비율에 변화가 생기지만, 그것들이 그렇게 명확하지는 않다.
...
1. sift, polyphen, MPC사이의 상관관계에 완벽하게 명확하지 않다.
...
print(p)
```
새로운 데이터 p는 SCN2A에 문제를 가진 환자들의 데이터이다. 즉, 이 데이터의 variant 타입(Chr, Pos_hg19, Ref, Alt)와 일치하는 d의 데이터는 환자로, 아니면 일반인으로 구분할 수 있을 것이다.

d1의 variant가 p에 존재하면 'patient', 그렇지 않으면 'normal'값을 지정하는 새로운 column 'disease'를 d1에 추가한다.
```{r echo=TRUE}
...
우선 3번 목차에서 MPC, SIFT, Polyphen은 연관성이 있다는 것을 확인했으므로 MPC와 gnomAD_AF의 관계만을 비교하도록 하겠다.

#### 3-1. data 정리
MPC, gnomAD_AF column만 선택하고, gnomAD_AF에 있는 NA와 MPC의 '-'는 정확한 의미를 알 수 없으므로 제외한 새로운 dataframe d2를 생성하도록 하겠다.
```{r echo=TRUE}
...
d2 %>% ggplot(aes(sample = gnomAD_AF)) +  geom_qq(dparams = params) + geom_abline() + scale_y_continuous(limits=c(-1, 1)) + scale_x_continuous(limits=c(-1, 1)) + labs(title = 'qqplot of gnomAD_AF')
```
...
exon과 MPC, SIFT, PolyPhen, disease의 관계를 비교해보고 싶었다.
...
d1의 sift_n column에 대하여, variant가 환자에 해당하는 경우는 기존의 sift_n값 대신 'patient'를 표시하는 새로운 column 'danger'를 포함한 dataframe d3를 만들고, 이에 따라 color를 지정하여 전체적으로 파악할 수 있게 하였다.
```{r}
danger <- c(1:770)
d3 <- ifelse(d1$disease[danger] == 'patient', print('patient'), print(d1$sift_c[danger])) %>% mutate(d1, danger = .)
```


```{r echo=TRUE}
d3 %>% ggplot(aes(as.numeric(as.character(MPC)), as.numeric(poly_n), col = danger)) + geom_point(size = 0.2) + facet_wrap(~EXON) + labs(title = 'relationship between exon and MPC,PolyPhen,SIFT, presence of disease', color = 'effect of variant(by SIFT)', x='MPC', y='PolyPhen')
```
이 그래프를 통해 알 수 있는 것은
...
# 5. 결론
위의 결과들을 바탕으로 data 분석 전 세웠던 탐구주제애 대하 모두 결론을 내릴 수 있게 되었다.
**1. variant의 표현형에 대한 effect와 관련된 column간의 상관관계 유무 탐구**
상관관계가 존재한다. MPC, PolyPhen, SIFT는 모두 연관되어 있으며, MPC,PolyPhen는 비례, 그리고 이 둘과 SIFT의 수치는 반비례하는 관계를 가진다.
...
ex) 27, 7은 낮은 값의 비율이 높은 반면 6, 16번은 높은 값의 비율이 높다.
MPC: 값의 분포가 각 exon마다 다르다.
ex) 16, 27은 높은 값의 비율이 높은 편이다.
...
**2. variant들은 exon에 걸쳐 고르게 분포하지 않는다**
